...
- An example app in the `example/` directory that uses the local source code via portal.
...
> The example app automatically uses the local library source code via portal, so you can test changes in real-time.
...
```sh
Running "ImageCodeScannerExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
